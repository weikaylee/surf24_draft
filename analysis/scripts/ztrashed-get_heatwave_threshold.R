...
for (year in yearly_input_data) {
  ...
  extracted_vals_sorted <- sort(unlist(extracted_vals_flattened), decreasing = FALSE)  
  yearly_output_data[[as.character(year)]] <- mean(extracted_vals_sorted)
}

# get 85th percentile 
yearly_output_data_sorted <- sort(unlist(yearly_output_data), decreasing = FALSE) 
...
print(percentile_val) # get 18.24078 -- pretty close to threshold computed in 05_heatwave_day_counts (18.8 ish)

# each grid is 770 m by 770 m 
# ideally do with population, ince dealing with people 
# sanity check: make sure all weights add up to one for each 
# output: dataframe 

# approach: get one raster for percent covered, then another for weight, 
# dont need air temp 
