...
yearly_data <- list() 
for (year in years) {
  yearly_data[[as.character(year)]] <- file.path(base_path, paste("LA_DTRNG", year, sep="_")) 
}
...
for (year in names(yearly_data)) {
    ...
    names(binned_raster) <- paste(raster_path_basename, bin_labels, sep="_")
...
for (year in names(yearly_data)) {
    ...
    raster_path <- file.path(output_path, paste0(paste("LA_DTRNG_BINS", year, sep="_"), ".tif"))
    ...
    if (all(values(num_days_raster) != 1)) {
      stop(paste("Actual total number of days across bins does not match expected for", year))
    }
}

# check that distribution and trends of bin counts, pixel vals match (are roughly normal)
test_files <- list.files(file.path("analysis", "processed", "la_dtrng", "LA_DTRNG_2003"), full.names=TRUE)
test_stack <- rast(test_files)
test_brick <- rast(file.path("analysis", "processed", "diurnal_range_bins", "LA_DTRNG_BINS_2003.tif"))
